...
bool sendMsg(const char* msg, DX_MESSAGE_PROPERTY** messageProperties, size_t messagePropertyCount);
...
typedef enum {
	...
	DEVICE_AWAITING_AUTHENTICATION,
...
IoTHubClientAuthenticationState_NotAuthenticated; // Authentication state with respect to the IoT Hub.
...
static DX_TIMER cloudToDeviceTimer = {
...
static void dx_azureToDeviceStart(void) {
	if (cloudToDeviceTimer.eventLoopTimer == NULL) {
		...
		dx_timerOneShotSet(&cloudToDeviceTimer, &(struct timespec){0, 250000000});
	}
}
...
void dx_azureConnectionStringSet(const char* connectionString) {
...
bool dx_azureIsConnected(void) {
	return iotHubClientAuthenticationState == IoTHubClientAuthenticationState_Authenticated && dx_isNetworkReady();
	//return connection_state == DEVICE_CONNECTED;
}
...
static void AzureCloudToDeviceHandler(EventLoopTimer* eventLoopTimer) {
	...
	if (dx_isNetworkReady()) {
		...
		case IoTHubClientAuthenticationState_Device_Disbled:
			if (period % 2 == 0) {
				// Doing a do work straight after a device disable seems to cause an issue
				// So do on even periods.
				IoTHubDeviceClient_LL_DoWork(iothubClientHandle);				
			}
			period++;
...
static bool SetupAzureClient() {
	...
	IoTHubDeviceClient_LL_SetDeviceMethodCallback(iothubClientHandle, dx_directMethodHandler, NULL);
