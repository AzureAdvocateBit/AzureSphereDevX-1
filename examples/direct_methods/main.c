...
static DX_TIMER restartDeviceOneShotTimer = { .period = {0, 0}, .name = "restartDeviceOneShotTimer", .handler = DelayRestartDeviceTimerHandler };

// Azure IoT Direct Methods
static DX_DIRECT_METHOD_BINDING dm_lightControl = { .methodName = "LightControl", .handler = LightControlHandler };
...
// Direct method name = LightControl, json payload = {"State": true, "Duration":2} or {"State": false, "Duration":2}
static DX_DIRECT_METHOD_RESPONSE_CODE LightControlHandler(JSON_Value* json, DX_DIRECT_METHOD_BINDING* directMethodBinding, char** responseMsg) {
...
static void InitPeripheralsAndHandlers(void) {
	...
	dx_directMethodSetOpen(directMethodBindingSet, NELEMS(directMethodBindingSet));
}
